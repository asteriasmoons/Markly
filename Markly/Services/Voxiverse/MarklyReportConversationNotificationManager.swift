...
enum MarklyReportConversationNotificationManager {
    ...
    private static let container = CKContainer(identifier: MarklyReportConversationCloudKitSchema.containerIdentifier)
    private static let zoneSubscriptionPrefix = "markly-report-message-events-v3-"
    private static let invitationSubscriptionID = "markly-report-invitations-v3"
    private static let processedMessagePrefix = "markly.reportConversation.processedMessage."
    private static let processedInvitationPrefix = "markly.reportConversation.processedInvitation."
    ...
    static func prepareNotifications() async {
        ...
        if let zones = try? await container.sharedCloudDatabase.allRecordZones() {
            for zone in zones where zone.zoneID.zoneName == MarklyReportConversationCloudKitSchema.zoneName {
                await ensureMessageSubscriptionLogged(database: container.sharedCloudDatabase, zoneID: zone.zoneID)
            }
        }
        let privateZoneID = CKRecordZone.ID(zoneName: MarklyReportConversationCloudKitSchema.zoneName, ownerName: CKCurrentUserDefaultName)
    ...
    static func reportNotifLog(_ message: String) {
        #if DEBUG
        print("[ReportNotif][Markly] \(message)")
        #endif
    }
    ...
    static func scanForPendingInvitations() async {
        ...
        let predicate = NSPredicate(format: "appID == %@", MarklyReportConversationCloudKitSchema.sourceAppID)
        let query = CKQuery(recordType: MarklyReportConversationCloudKitSchema.RecordType.report, predicate: predicate)
    ...
    static func processInvitation(recordID: CKRecord.ID) async -> Bool {
        ...
        guard let record = try? await database.record(for: recordID),
              record.marklyString("appID") == MarklyReportConversationCloudKitSchema.sourceAppID,
              record.marklyString(MarklyReportConversationCloudKitSchema.PublicReportField.conversationState) == MarklyReportConversationState.invited.rawValue else { return false }
        let conversationID = record.marklyString(MarklyReportConversationCloudKitSchema.PublicReportField.conversationRecordName)
        ...
        let reportID = record.marklyString("reportID").isEmpty ? recordID.recordName : record.marklyString("reportID")
        ...
        do {
            try await UNUserNotificationCenter.current().add(UNNotificationRequest(identifier: "markly-report-conversation-invite-\(conversationID)", content: content, trigger: nil))
            return true
        } catch {
            releaseNotification(processedKey)
            return false
    ...
    private static func ensureInvitationSubscription() async throws {
        ...
        let predicate = NSPredicate(format: "appID == %@", MarklyReportConversationCloudKitSchema.sourceAppID)
        let subscription = CKQuerySubscription(recordType: MarklyReportConversationCloudKitSchema.RecordType.report, predicate: predicate, subscriptionID: invitationSubscriptionID, options: [.firesOnRecordUpdate])
    ...
    private static func ensureMessageSubscription(database: CKDatabase, zoneID: CKRecordZone.ID) async throws {
        ...
        let oldSubscriptionID = "markly-report-conversations-\(zoneID.ownerName)-\(zoneID.zoneName)"
    ...
    private static func deviceSubscriptionFlagKey(_ subscriptionID: String) -> String {
        "markly.reportConversation.deviceSubscriptionSaved.\(subscriptionID)"
    }

    private static func notifyForNewStaffMessage(_ message: CKRecord, database: CKDatabase) async -> Bool {
        guard message.recordType == MarklyReportConversationCloudKitSchema.RecordType.message,
              message.marklyString(MarklyReportConversationCloudKitSchema.MessageField.senderRole) == MarklyReportConversationSenderRole.staff.rawValue else { return false }
        let messageID = message.marklyString(MarklyReportConversationCloudKitSchema.MessageField.messageID).isEmpty ? message.recordID.recordName : message.marklyString(MarklyReportConversationCloudKitSchema.MessageField.messageID)
        let processedKey = processedMessagePrefix + messageID
        guard claimNotification(processedKey) else { return false }
        let rootID = (message[MarklyReportConversationCloudKitSchema.MessageField.conversation] as? CKRecord.Reference)?.recordID
            ?? CKRecord.ID(recordName: message.marklyString(MarklyReportConversationCloudKitSchema.MessageField.conversationRecordName), zoneID: message.recordID.zoneID)
        guard let root = try? await database.record(for: rootID),
              root.marklyString(MarklyReportConversationCloudKitSchema.ConversationField.sourceAppID) == MarklyReportConversationCloudKitSchema.sourceAppID,
              root.marklyString(MarklyReportConversationCloudKitSchema.ConversationField.invitationState) != MarklyReportConversationState.invited.rawValue else {
            releaseNotification(processedKey)
            return false
        }
        guard await requestAuthorizationIfNeeded() else {
        ...
        let reportID = root.marklyString(MarklyReportConversationCloudKitSchema.ConversationField.reportID)
        let content = UNMutableNotificationContent()
        content.title = "Voxiverse"
        ...
        do {
            try await UNUserNotificationCenter.current().add(UNNotificationRequest(identifier: "markly-report-conversation-message-\(messageID)", content: content, trigger: nil))
            return true
        } catch {
    ...
    private static func changeTokenKey(for zoneID: CKRecordZone.ID) -> String {
        "markly.reportConversation.changeToken.\(zoneID.ownerName).\(zoneID.zoneName)"
    }
...
final class MarklyNotificationDelegate: NSObject, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    func application(
        ...
        Task { await MarklyReportConversationNotificationManager.prepareNotifications() }
        return true
    }
    ...
    func application(
        ...
        guard let notification = CKNotification(fromRemoteNotificationDictionary: userInfo) else {
            MarklyReportConversationNotificationManager.reportNotifLog("remote notification received but could not decode CKNotification")
        ...
        Task {
            ...
            if notification.subscriptionID == "markly-report-invitations-v3",
               let recordID = (notification as? CKQueryNotification)?.recordID {
                notified = await MarklyReportConversationNotificationManager.processInvitation(recordID: recordID)
            } else if let zoneNotification = notification as? CKRecordZoneNotification,
                      let zoneID = zoneNotification.recordZoneID,
                      zoneID.zoneName == MarklyReportConversationCloudKitSchema.zoneName {
                notified = await MarklyReportConversationNotificationManager.processMessageChanges(in: zoneID, databaseScope: zoneNotification.databaseScope)
            } else {
                completionHandler(.noData)
                return
            }
