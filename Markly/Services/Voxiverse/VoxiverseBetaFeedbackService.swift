...
final class VoxiverseBetaFeedbackService {
    ...
    @MainActor
    func submit(_ payload: VoxiverseBetaFeedbackPayload) async throws -> (reportID: String, diagnostics: MarklyReportDiagnostics) {
        let trimmedTitle = payload.title.trimmed
        let trimmedArea = payload.area.trimmed
        let trimmedOverallExperience = payload.overallExperience.trimmed
        let trimmedTestedWhat = payload.testedWhat.trimmed
        ...
        let diagnostics = MarklyReportDiagnostics.current(screenName: "Settings > Beta Feedback")
        ...
        record["workedWell"] = payload.workedWell.trimmed as CKRecordValue
        record["couldBeBetter"] = payload.couldBeBetter.trimmed as CKRecordValue
        record["anythingUnexpected"] = payload.unexpected.trimmed as CKRecordValue
        record["internalNotes"] = payload.additionalThoughts.trimmed as CKRecordValue
        record["status"] = "New" as CKRecordValue
        applyDiagnostics(diagnostics, to: record)

        let tempURLs = try makeAttachmentFiles(from: payload.attachmentData, reportID: reportID, folderName: "VoxiverseMarklyBetaFeedback")
